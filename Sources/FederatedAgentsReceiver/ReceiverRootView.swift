...
struct ReceiverRootView: View {
    ...
    private var invitationBanner: some View {
        VStack(spacing: 10) {
            ForEach(model.pendingInvitations, id: \.id) { invitation in
                InvitationBannerCard(
                    invitation: invitation,
                    onAccept: { model.acceptInvitation(invitation) },
                    onDismiss: { model.dismissInvitation(invitation) }
                )
            }
        }
...
private struct InvitationBannerCard: View {
    let invitation: IncomingInvitation
    let onAccept: () -> Void
    let onDismiss: () -> Void
    ...
    private var subtitle: String {
        var parts = ["From \(invitation.senderName)"]
        if let organization = invitation.senderOrganization, !organization.isEmpty {
            parts.append(organization)
        }
        parts.append(invitation.summary)
